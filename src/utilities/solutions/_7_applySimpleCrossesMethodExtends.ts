...
import { simpleCrossesMethodExtends } from '@/utilities/solutions/_7_simpleCrossesMethodExtends';
import {
  cloneClueInfo,
  getRowStates,
  setRowStates,
...
export function applySimpleCrossesMethodExtends(model: NonogramGridModel): {
  ...
  while (progress) {
    ...
    for (let rowIndex = 0; rowIndex < nextModel.rows; rowIndex++) {
      const result = simpleCrossesMethodExtends(
